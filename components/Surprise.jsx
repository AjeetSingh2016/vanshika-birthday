...
const EnhancedTwoLayerCake = () => {
  const flameVariants = {
    animate: {
      ...
      filter: ["brightness(100%)", "brightness(115%)", "brightness(90%)", "brightness(110%)", "brightness(100%)"],
      transition: {
        duration: 1.5,
        repeat: Infinity,
        repeatType: "loop",
  ...
  return (
    <div className="flex min-h-[600px] items-center justify-center p-4 text-black">
      <div className="relative w-full max-w-md md:max-w-lg lg:max-w-xl">
        ...
        <div className="absolute top-1/2 left-1/2 w-11/12 -translate-x-1/2 -translate-y-[20px] transform">
          <div className="relative h-36 w-full rounded-3xl border-4 border-amber-400 bg-gradient-to-b from-yellow-200 to-amber-300 shadow-md">
            ...
            <div className="absolute left-0 top-4 h-28 w-full overflow-hidden opacity-25">
              {[...Array(14)].map((_, i) => (
                <div key={`texture-b-${i}`} className="absolute h-px w-full bg-amber-800" style={{ top: i * 6 }}></div>
              ))}
            </div>
            ...
            >
              <Link href="/puzzle">
                <button className="transform rounded-full bg-pink-600 px-6 py-2 text-lg font-bold text-white shadow-md transition-all duration-300 hover:scale-105 hover:bg-pink-700">
                  Click to Begin
                </button>
              </Link>
            </motion.div>
        ...
        <div className="absolute top-32 left-1/2 w-10/12 -translate-x-1/2 -translate-y-[156px] transform z-50">
          ...
          {/* Icing drips */}
          {Array.from({ length: 15 }, (_, i) => i * 6.66).map((percentage, i) => (
            <div
              key={`drip-${i}`}
        ...
        <div className="absolute top-1/3 left-1/2 w-9/12 -translate-x-1/2 -translate-y-[164px] transform">
          <div className="h-36 w-full rounded-t-2xl border-4 border-rose-400 border-b-0 bg-gradient-to-b from-rose-200 to-rose-300 shadow-md ">
            ...
            <div className="absolute left-0 top-4 h-24 w-full overflow-hidden opacity-25">
              {[...Array(12)].map((_, i) => (
                <div key={`texture-t-${i}`} className="absolute h-px w-full bg-rose-800" style={{ top: i * 6 }}></div>
              ))}
            </div>
            {/* Top icing */}
            ...
            <div className="absolute left-0 top-0 w-full">
              {Array.from({ length: 13 }, (_, i) => i * 8.33).map((percentage, i) => (
                <div
                  key={`swirl-${i}`}
                  className="absolute -top-4 rounded-full bg-white shadow-sm"
                  style={{ left: `${percentage}%`, width: '8px', height: '8px' }}
                ></div>
              ))}
            </div>
            {/* Sprinkles */}
            {[...Array(25)].map((_, i) => (
              <div
                ...
                className={`absolute rounded-full w-2 h-2 shadow-sm ${
                  ['bg-pink-500', 'bg-yellow-500', 'bg-blue-500', 'bg-purple-500', 'bg-green-500'][i % 5]
                }`}
                style={{
        ...
        {[15, 28, 40, 55, 70, 80].map((percentage, i) => (
          <div key={`candle-${i}`} className="absolute top-1/2 z-40 -translate-y-[244px]" style={{ left: `${percentage}%` }}>
            <div
              className={`h-20 w-4 rounded-t-md shadow-md ${
                ['bg-red-400', 'bg-yellow-400', 'bg-blue-400', 'bg-green-400', 'bg-purple-400', 'bg-pink-400'][i % 6]
              }`}
            ></div>
